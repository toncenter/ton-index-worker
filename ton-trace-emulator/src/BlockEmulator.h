...
private:
    ...
    void trace_emulated(std::unique_ptr<Trace> trace);
    void trace_finished(TraceId trace_id);

    td::Result<block::Account> fetch_account(const block::StdAddress& addr, ton::UnixTime now);

public:
    McBlockEmulator(MasterchainBlockDataState mc_data_state, std::function<void(std::unique_ptr<Trace>, td::Promise<td::Unit>)> trace_processor, td::Promise<> promise);
