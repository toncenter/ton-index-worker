  ...
  std::vector<std::tuple<td::Bits256, std::string>> msg_bodies;
  {
    ...
    auto store_message = [&](const schema::Transaction& tx, const schema::Message& msg, std::string direction) {
      ...
      query << "("
            ...
            << txn.quote(direction) << ","
            << TO_SQL_OPTIONAL_STRING(msg.source) << ","
            << TO_SQL_OPTIONAL_STRING(msg.destination) << ","
            << to_int64(msg.value) << ","
    ...
    try {
      ...
      }
      {
        ...
        std::string query = (
          ...
          "create type blockidext as (workchain integer, shard bigint, seqno integer, root_hash varchar(44), file_hash varchar(44));\n"
